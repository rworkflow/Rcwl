...
The `Rpipe` package is aimed to be a simple and user-friendly way to manage command line tools in R and build data analysis pipelines using Common Workflow Language (CWL). It can be a bridge between heavy bioinformatics tools and pipeline to R/Bioconductor. More details about CWL can be found at [http://www.commonwl.org]().
...
First, we need to define the input parameter for "echo", a string without a prefix. Just an `id` option required.
```{r}
...
Let's run it. A list including the command executed, temporary output and logs. All standard out and standard error stream can also be printed by setting `stderr = ""`.
```{r}
r1 <- runCWL(echo)
r1
readLines(r1$output)
...
Here is an example to uzip a compressed `gz` file. First, we generate a compressed R script file.
```{r}
...
GZ <- cwlParam(baseCommand = c("gzip", "-d", "-c"),
               requirements = list(class = "InlineJavascriptRequirement"),
               inputs = InputParamList(z1),
...
The CWL can work with docker to simplify your software management and communicate files between host and container. The docker container can be defined by the `hints` option.
```{r, eval=FALSE}
d1 <- InputParam(id = "rfile", type = "File")
doc <- cwlParam(baseCommand = "Rscript",
                inputs = InputParamList(d1),
                stdout = "output.txt",
                hints = list(DockerRequirement = list(dockerPull = "r-base")))
doc$rfile <- r4$output
r6 <- runCWL(doc)
```

# Running Tools in Cluster server
The CWL also can work in high performance clusters with batch-queuing system, such as SGE, PBS, SLURM and so on, using the Bioconductor package `BiocParallel`. Here is an example to submit jobs with SGE.
...
results <- bplapply(letters[1:4], testRun, BPPARAM = param1, cwl=echo)
...
results <- bplapply(letters[1:4], testRun, BPPARAM = param, cwl=echo)
results <- bplapply(letters[1:4], test, BPPARAM = param)
...
```{r}
d1 <- InputParam(id = "rfile", type = "File")
o1 <- OutputParam(id = "Rout", type = "File", glob = "output.txt")
...
o1 <- OutputParam(id = "cwl_cout", type = "File", outputSource = "Compile/Rout")
```
