...
`cwlParam` object and the `In` option in steps should be linked to the
input parameters defined by `cwlStepParam`.
...
The `cwlStepParam` is used to collect `inputs` and `outputs` from
