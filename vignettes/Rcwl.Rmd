...
The pipeline includes two steps, decompressed by the predefined tool
`GZ` and compiled by `Rs`. The input file is a compressed file for the
first "Uncomp" step. 
...
The input "cwl_zfile", it is assigned to the `GZ` input `zfile` ih the
frist step. Then the input of 'Rs' in the "Compile" step is assigned
